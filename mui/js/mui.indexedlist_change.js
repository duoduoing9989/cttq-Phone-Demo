...
(function($, window, document) {
	...
	var IndexedList = $.IndexedList = $.Class.extend({
		...
		caleLayout: function() {
			...
			document.getElementById("content").style.height = withoutSearchHeight;
		...
		scrollTo: function(group) {
			...
			self.el.inner.scrollTop = groupElement.offsetTop + document.getElementById("hotCityBox").offsetHeight + 2;
		},
		...
		search: function(keyword) {
			...
			} else if (selectorBuffer.length > 0 && keyword != "") {
			...
			styleForSearch = self.el.styleForSearch.innerText;
		},
	...
	var searchList = $.searchList = $.Class.extend({
		/**
		 * 通过 element 和 options 构造 IndexedList 实例
		 **/
		init: function(holder, options) {
