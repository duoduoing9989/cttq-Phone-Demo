...
+function ($) {
    ...
    $.pickerModal = function (pickerModal, removeOnClose, locationClass) {
        if (typeof removeOnClose === 'undefined') removeOnClose = true;
        if (typeof pickerModal === 'string' && pickerModal.indexOf('<') >= 0) {
            ...
            if (pickerModal.length > 0) {
                ...
                if( locationClass ){
                    $('.'+locationClass+'').append(pickerModal[0]);
                }else{
                    $(defaults.modalContainer).append(pickerModal[0]);
                }
                // $(window.parent.document).find("body").append(pickerModal[0]);
            }
            else return false; //nothing found
        }
        pickerModal = $(pickerModal);
...
+function ($) {
    ...
    var Calendar = function (params) {
        ...
        if( params.btnShow == true ){
            var monthPickerTemplateHTML = '<div class="calendarBtn ub ub-ac ub-pc ub-f1 calendarClose">清空</div>' +
                '<div class="picker-calendar-month-picker">' +
                '<a href="#" class="link icon-only picker-calendar-prev-month"><i class="icon icon-prev"></i></a>' +
                '<div class="current-month-value ub ub-ac ub-pc"></div>' +
                '<a href="#" class="link icon-only picker-calendar-next-month"><i class="icon icon-next"></i></a>' +
                '</div>' +
                '<div class="calendarBtn ub ub-ac ub-pc ub-f1 calendarComplete">完成</div>';
        }else{
            var monthPickerTemplateHTML = '<div class="picker-calendar-month-picker">' +
                '<a href="#" class="link icon-only picker-calendar-prev-month"><i class="icon icon-prev"></i></a>' +
                '<div class="current-month-value ub ub-ac ub-pc"></div>' +
                '<a href="#" class="link icon-only picker-calendar-next-month"><i class="icon icon-next"></i></a>' +
                '</div>'
        ...
        var defaults = {
            ...
            closeOnSelect: true,
            monthPicker: true,
            monthPickerTemplate: monthPickerTemplateHTML,
            yearPicker: true,
            yearPickerTemplate:
                '<div class="picker-calendar-year-picker" style="display: none;">' +
                '<a href="#" class="link icon-only picker-calendar-prev-year"><i class="icon icon-prev"></i></a>' +
                '<span class="current-year-value ub ub-ac ub-pc"></span>' +
            ...
            toolbarTemplate:
                '<div class="toolbar ub ub-ac">' +
                '<div class="toolbar-inner ub ub-pc ub-f1">' +
                '{{yearPicker}}' +
                '{{monthPicker}}' +
        ...
        p.addValue = function (value) {
            if (p.params.multiple) {
                ...
                if( p.value.length == 2 ){
                    rangeValue = [];
                    daysDiffer = (p.value[1] - p.value[0])/oneDayTime;
                    rangeValue.push(p.value[0]);
                    for( var i = 1; i < daysDiffer; i++ ){
                        var sum = p.value[0] + oneDayTime*i;
                        rangeValue.push(sum);
                    }
                    rangeValue.push(p.value[1]); 
                }else{
                    rangeValue = [];
                    rangeValue.push(p.value[0]);
                }
                p.updateValue(rangeValue);
            }
            else {
                p.value = [value];
                p.updateValue();
        ...
        p.updateValue = function (rangeValue) {
            ...
            var i, inputValue;
            if ( p.params.multiple ) {
                $(".rangeStart").html($(".rangeStart").attr("text"));
                $(".rangeStart").removeClass("rangeStart");
                $(".rangeEnd").html($(".rangeEnd").attr("text"));
                $(".rangeEnd").removeClass("rangeEnd");
                $(".picker-calendar-day-selectedBig").each(function(){
                    $(this).find("b").last().html($(this).find("b").last().attr("text"));
                });
                $(".picker-calendar-day-selectedBig").removeClass("picker-calendar-day-selectedBig");
                if( rangeValue.length == 2 && rangeValue[0] == rangeValue[1] ){
                    var valueDate = new Date(rangeValue[1]);
                    var dom = p.wrapper.find('.picker-calendar-day[data-date="' + valueDate.getFullYear() + '-' + valueDate.getMonth() + '-' + valueDate.getDate() + '"]');
                    dom.addClass('picker-calendar-day-selected');
                    ...
                    for (i = 0; i < rangeValue.length; i++) {
                        var valueDate = new Date(rangeValue[i]);
                        var dom = p.wrapper.find('.picker-calendar-day[data-date="' + valueDate.getFullYear() + '-' + valueDate.getMonth() + '-' + valueDate.getDate() + '"]');
                        dom.addClass('picker-calendar-day-selected');
                        if( i == 0 ){
                            dom.find("b.lunar").html("开始");
                            dom.find("b.lunar").addClass("rangeStart");
                        }else if( rangeValue.length > 1 && i == rangeValue.length - 1 ){
                            dom.find("b.lunar").html("结束");
                            dom.find("b.lunar").addClass("rangeEnd");
                            if( p.params.btnShow == false ){
                                setTimeout(function(){
                                    if (p.params.closeOnSelect) p.close();
                            ...
                            dom.addClass('picker-calendar-day-selectedBig');
                            dom.find("b.lunar").html("");
                        }
                    }
                }
            }else{
        ...
        p.initCalendarEvents = function () {
            ...
            //新增取消按钮
            $(".calendarClose").on('click', calendarClose);
            ...
            $(".calendarComplete").on('click', calendarComplete);
            //新增完成按钮
            function calendarComplete(){
                if( p.params.multiple ){
                    if( p.value ){
                        ...
                        p.params.onChange(p, p.value, '完成');
                        p.close();
                    }
                }else{
                    p.params.onChange(p, p.value, p.value.map(formatDate));
        ...
        p.monthHTML = function (date, offset) {
            ...
            if ( p.params.multiple ) {
                for (i = 0; i < rangeValue.length; i++) {
                    currentValues.push(new Date(rangeValue[i]).setHours(0,0,0,0));
                }
            }else{
            ...
            for (i = 1; i <= rows; i++) {
                ...
                for (j = 1; j <= cols; j++) {
                    ...
                    var equalTag = false;
                    if( rangeValue.length == 2 && rangeValue[0] == rangeValue[1] && dayDate == new Date(rangeValue[0]).setHours(0,0,0,0) ){
                        equalTag = true
                    }
                    var todayTag = false;
                    // Today
                    ...
                    //Start
                    if( dayDate === new Date(rangeValue[0]).setHours(0,0,0,0) ){
                        startDayTag = true;
                    }
                    var endDayTag = false;
                    //End
                    if( dayDate === new Date(rangeValue[rangeValue.length-1]).setHours(0,0,0,0) ){
                        endDayTag = true;
                    }
                    ...
                    var LunarCalendar = cttqAI_GetCalendarLunar(dayYear,dayMonth,dayNumber);
                    if (todayTag === true){
                        if( equalTag == true ){
                            rowHTML += '<div data-year="' + dayYear + '" data-month="' + dayMonth + '" data-day="' + dayNumber + '" class="picker-calendar-day ub ub-ac ub-pc' + (addClass) + '" data-date="' + (dayYear + '-' + dayMonth + '-' + dayNumber) + '"><span class="ub ub-ac ub-pc ub-ver"><b class="ub ub-ac ub-pc number today '+LunarCalendar.class+'">今天</b><b class="ub ub-ac ub-pc lunar today tag rangeStart '+LunarCalendar.class+'" text='+LunarCalendar.text+'>同天</b></span></div>';
                        }else{
                            if(startDayTag === true){
                                rowHTML += '<div data-year="' + dayYear + '" data-month="' + dayMonth + '" data-day="' + dayNumber + '" class="picker-calendar-day ub ub-ac ub-pc' + (addClass) + '" data-date="' + (dayYear + '-' + dayMonth + '-' + dayNumber) + '"><span class="ub ub-ac ub-pc ub-ver"><b class="ub ub-ac ub-pc number today '+LunarCalendar.class+'">今天</b><b class="ub ub-ac ub-pc lunar today tag rangeStart '+LunarCalendar.class+'" text='+LunarCalendar.text+'>开始</b></span></div>';
                            }else if(endDayTag === true){
                                rowHTML += '<div data-year="' + dayYear + '" data-month="' + dayMonth + '" data-day="' + dayNumber + '" class="picker-calendar-day ub ub-ac ub-pc' + (addClass) + '" data-date="' + (dayYear + '-' + dayMonth + '-' + dayNumber) + '"><span class="ub ub-ac ub-pc ub-ver"><b class="ub ub-ac ub-pc number today '+LunarCalendar.class+'">今天</b><b class="ub ub-ac ub-pc lunar today tag rangeEnd '+LunarCalendar.class+'" text='+LunarCalendar.text+'>结束</b></span></div>';
                            }else if( selectedTag == true ){
                                rowHTML += '<div data-year="' + dayYear + '" data-month="' + dayMonth + '" data-day="' + dayNumber + '" class="picker-calendar-day ub ub-ac ub-pc' + (addClass) + ' picker-calendar-day-selectedBig" data-date="' + (dayYear + '-' + dayMonth + '-' + dayNumber) + '"><span class="ub ub-ac ub-pc ub-ver"><b class="ub ub-ac ub-pc number today '+LunarCalendar.class+'">今天</b><b class="ub ub-ac ub-pc lunar today tag '+LunarCalendar.class+'" text='+LunarCalendar.text+'></b></span></div>';
                            }else{
                                rowHTML += '<div data-year="' + dayYear + '" data-month="' + dayMonth + '" data-day="' + dayNumber + '" class="picker-calendar-day ub ub-ac ub-pc' + (addClass) + '" data-date="' + (dayYear + '-' + dayMonth + '-' + dayNumber) + '"><span class="ub ub-ac ub-pc ub-ver"><b class="ub ub-ac ub-pc number today '+LunarCalendar.class+'">今天</b><b class="ub ub-ac ub-pc lunar today tag '+LunarCalendar.class+'" text='+LunarCalendar.text+'>'+LunarCalendar.text+'</b></span></div>';
                            }
                        }
                    }else{
                        if( equalTag == true ){
                            rowHTML += '<div data-year="' + dayYear + '" data-month="' + dayMonth + '" data-day="' + dayNumber + '" class="picker-calendar-day ub ub-ac ub-pc' + (addClass) + '" data-date="' + (dayYear + '-' + dayMonth + '-' + dayNumber) + '"><span class="ub ub-ac ub-pc ub-ver"><b class="ub ub-ac ub-pc number '+LunarCalendar.class+'">'+dayNumber+'</b><b class="ub ub-ac ub-pc lunar tag rangeStart '+LunarCalendar.class+'" text='+LunarCalendar.text+'>同天</b></span></div>';
                        }else{
                            if(startDayTag === true){
                                rowHTML += '<div data-year="' + dayYear + '" data-month="' + dayMonth + '" data-day="' + dayNumber + '" class="picker-calendar-day ub ub-ac ub-pc' + (addClass) + '" data-date="' + (dayYear + '-' + dayMonth + '-' + dayNumber) + '"><span class="ub ub-ac ub-pc ub-ver"><b class="ub ub-ac ub-pc number '+LunarCalendar.class+'">'+dayNumber+'</b><b class="ub ub-ac ub-pc lunar tag rangeStart '+LunarCalendar.class+'" text='+LunarCalendar.text+'>开始</b></span></div>';
                            }else if(endDayTag === true){
                                rowHTML += '<div data-year="' + dayYear + '" data-month="' + dayMonth + '" data-day="' + dayNumber + '" class="picker-calendar-day ub ub-ac ub-pc' + (addClass) + '" data-date="' + (dayYear + '-' + dayMonth + '-' + dayNumber) + '"><span class="ub ub-ac ub-pc ub-ver"><b class="ub ub-ac ub-pc number '+LunarCalendar.class+'">'+dayNumber+'</b><b class="ub ub-ac ub-pc lunar tag rangeEnd '+LunarCalendar.class+'" text='+LunarCalendar.text+'>结束</b></span></div>';
                            }else if(selectedTag == true){
                                rowHTML += '<div data-year="' + dayYear + '" data-month="' + dayMonth + '" data-day="' + dayNumber + '" class="picker-calendar-day ub ub-ac ub-pc' + (addClass) + ' picker-calendar-day-selectedBig" data-date="' + (dayYear + '-' + dayMonth + '-' + dayNumber) + '"><span class="ub ub-ac ub-pc ub-ver"><b class="ub ub-ac ub-pc number '+LunarCalendar.class+'">'+dayNumber+'</b><b class="ub ub-ac ub-pc lunar tag '+LunarCalendar.class+'" text='+LunarCalendar.text+'></b></span></div>';
                            }else{
                                rowHTML += '<div data-year="' + dayYear + '" data-month="' + dayMonth + '" data-day="' + dayNumber + '" class="picker-calendar-day ub ub-ac ub-pc' + (addClass) + '" data-date="' + (dayYear + '-' + dayMonth + '-' + dayNumber) + '"><span class="ub ub-ac ub-pc ub-ver"><b class="ub ub-ac ub-pc number '+LunarCalendar.class+'">'+dayNumber+'</b><b class="ub ub-ac ub-pc lunar tag '+LunarCalendar.class+'" text='+LunarCalendar.text+'>'+LunarCalendar.text+'</b></span></div>';
                            }
                        } 
                    }
                }
                monthHTML += '<div class="picker-calendar-row ub">' + rowHTML + '</div>';
            }
            monthHTML = '<div class="picker-calendar-month ub ub-ver" data-year="' + year + '" data-month="' + month + '">' + monthHTML + '</div>';
            return monthHTML;
        };
        p.animating = false;
        p.updateCurrentMonthYear = function (dir) {
            ...
            p.container.find('.current-month-value').text(''+p.currentYear+'年 '+p.params.monthNames[p.currentMonth]+'');
        ...
        p.layout = function () {
            ...
            var monthsHTML = '<div class="picker-calendar-months ub"><div class="picker-calendar-months-wrapper ub">' + (prevMonthHTML + currentMonthHTML + nextMonthHTML) + '</div></div>';
            // Week days header
            var weekHeaderHTML = '';
            if (p.params.weekHeader) {
                for (i = 0; i < 7; i++) {
                    ...
                    weekHeaderHTML += '<div class="picker-calendar-week-day ub ub-ac ub-pc ' + ((p.params.weekendDays.indexOf(weekDayIndex) >= 0) ? 'picker-calendar-week-day-weekend' : '') + '"> ' + dayName + '</div>';

                }
                weekHeaderHTML = '<div class="picker-calendar-week-days ub ub-ac">' + weekHeaderHTML + '</div>';
            }
            pickerClass = 'picker-modal ub ub-ver picker-calendar ' + (p.params.cssClass || '');
            ...
            pickerHTML =
                ...
                toolbarHTML +
                '<div class="picker-modal-inner ub ub-ver">' +
                weekHeaderHTML +
                monthsHTML +
                '</div>' +
                '</div>';
        ...
        p.open = function () {
            ...
            if (!p.opened) {
                if( p.params.costValue == true ){
                    p.params.values = [];
                    p.params.values.push(p.input.parent().attr("value"))
                }
                if( Utils.cache.get("calendarClear") == "true" ){
                    delete p.value;
                    Utils.cache.del("calendarClear");
                }
                ...
                // Update input value
                if (updateValue) p.updateValue(rangeValue);
            }
    ...
    var Picker = function (params) {
        ...
        p.columnHTML = function (col, onlyItems) {
            ...
            if (col.divider) {
                ...
                if( p.params.locationClass ){
                    for (var j = 0; j < col.values.length; j++) {
                        var thisVal = (col.displayValues ? col.displayValues[j] : col.values[j]);
                        if( thisVal.indexOf('M') > -1 ){
                            // if( j == 0 ){
                            //     var valueArr = thisVal.split("Y");
                            //     valueArr[1] = valueArr[1].split("M");
                            //     valueArr[1][1] = valueArr[1][1].split("D");
                            //     valueArr[1][1][1] = valueArr[1][1][1].split("W");
                            //     columnItemsHTML += '<div class="picker-item picker-selected" data-picker-value="' + col.values[j] + '">'
                            //         +'<div class="ub ub-ac ub-pc pickerBox">'
                            //             +'<div class="ub ub-ac ub-pc ub-ver pickerBoxDate">'
                            //                 +'<div class="ub ub-ac ub-pc pickerBoxDateDay">'+valueArr[1][1][0]+'</div>'
                            //                 +'<div class="ub ub-ac ub-pc pickerBoxDateMonth">'+valueArr[1][0]+'</div>'
                            //             +'</div>'
                            //             +'<div class="ub ub-ac pickerBoxWeek">'+valueArr[1][1][1][0]+'</div>'
                            //             if( valueArr[1][1][1][1] != "" ){
                            //                 columnItemsHTML +='<div class="ub ub-ac pickerBoxText">'+valueArr[1][1][1][1]+'</div>'
                            //             }
                            //         columnItemsHTML +='</div>'
                            //     +'</div>';
                            // }else{
                                ...
                                valueArr[1][1] = valueArr[1][1].split("D");
                                valueArr[1][1][1] = valueArr[1][1][1].split("W");
                                valueArr[1][1][1][1] = valueArr[1][1][1][1].split("•");
                                columnItemsHTML += '<div class="picker-item" data-picker-value="' + col.values[j] + '">'
                                    +'<div class="ub ub-ac ub-pc pickerBox">'
                                        ...
                                        +'<div class="ub ub-ac pickerBoxWeek">'+valueArr[1][1][1][0]+'</div>'
                                        if( valueArr[1][1][1][1] != "" ){
                                            columnItemsHTML +='<div class="ub ub-ac pickerBoxDian">•</div><div class="ub ub-ac pickerBoxText">'+valueArr[1][1][1][1][1]+'</div>'
                                        }
                                    columnItemsHTML +='</div>'
                                +'</div>';
                            //}
                        }else{
                            columnItemsHTML += '<div class="picker-item ordinary" data-picker-value="' + col.values[j] + '">' + (col.displayValues ? col.displayValues[j] : col.values[j]) + '</div>';
                        }
                    }
                }else{
                    for (var j = 0; j < col.values.length; j++) {
        ...
        p.layout = function () {
            ...
            pickerHTML =
                ...
                colsHTML +
                '<div class="picker-center-highlight ub ub-ac">'+
                    '<div class="hours ub ub-ac ub-pc">时</div>'+
                    '<div class="minutes ub ub-ac ub-pc">分</div>'+
                '</div>' +
                '</div>' +
                '</div>';
        ...
        p.open = function () {
            if (!p.opened) {
                ...
                if (p.inline) {
                    ...
                    p.container = $($.pickerModal(p.pickerHTML,'',p.params.locationClass));
                    $(p.container)
                        .one('opened', function() {
                            p.opened = true;
                        })
        ...
        //回到今天
        p.goBackToday = function(){
            p.container.find('.picker-items-col').eq(0).each(function () {
                var updateItems = true;
                if ((!p.initialized && p.params.value) || (p.initialized && p.value)) updateItems = false;
                p.initPickerCol(this, updateItems);
            });
            p.container.find('.picker-items-col').eq(0).find('.picker-item').eq(0).addClass("picker-selected");
        };
